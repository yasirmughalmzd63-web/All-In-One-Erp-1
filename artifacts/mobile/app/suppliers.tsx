...
type Supplier = { id: number; name: string; phone?: string | null; email?: string | null; address?: string | null; balance: string };
...
export default function SuppliersScreen() {
  ...
  const openEdit = (s: Supplier) => { setEditItem(s); setForm({ name: s.name, phone: s.phone ?? "", email: s.email ?? "", address: s.address ?? "" }); setShowModal(true); };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.searchRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <TextInput style={[styles.searchInput, { color: colors.text }]} placeholder="Search suppliers..." placeholderTextColor={colors.mutedForeground} value={search} onChangeText={setSearch} />
      </View>
      ...
      {isLoading ? <ActivityIndicator style={{ margin: 40 }} color={colors.primary} /> : (
        <FlatList
          ...
          ListEmptyComponent={<View style={{ alignItems: "center", padding: 40 }}><Feather name="truck" size={40} color={colors.mutedForeground} /><Text style={{ fontFamily: "Inter_400Regular", color: colors.mutedForeground, marginTop: 12 }}>No suppliers</Text></View>}
          renderItem={({ item: s }) => (
            <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <View style={styles.cardMain}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {s.email && <Text style={[styles.cardSub, { color: colors.mutedForeground }]}>{s.email}</Text>}
                </View>
                <View style={{ gap: 8 }}>
                  {isAdmin && <TouchableOpacity style={[styles.actionBtn, { backgroundColor: colors.secondary }]} onPress={() => openEdit(s)}><Feather name="edit-2" size={14} color={colors.primary} /></TouchableOpacity>}
                  {isAdmin && <TouchableOpacity style={[styles.actionBtn, { backgroundColor: colors.dangerBg }]} onPress={() => handleDelete(s)}><Feather name="trash-2" size={14} color={colors.danger} /></TouchableOpacity>}
                </View>
              </View>
            </View>
          )}
