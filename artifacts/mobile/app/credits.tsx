...
export default function CreditsScreen() {
  ...
  const totalReceivable = credits.filter(c => c.type === "receivable" && c.status !== "paid").reduce((s, c) => s + parseFloat(c.remainingAmount), 0);
  const totalPayable = credits.filter(c => c.type === "payable" && c.status !== "paid").reduce((s, c) => s + parseFloat(c.remainingAmount), 0);
  const totalReceived = credits.filter(c => c.status === "paid" && c.type === "receivable").reduce((s, c) => s + parseFloat(c.amount), 0);
  ...
  const handlePay = async () => {
    ...
    try {
      await (payMut as unknown as { mutateAsync: (a: { id: number; data: unknown }) => Promise<unknown> }).mutateAsync({
        id: selected.id, data: { payAmount: parseFloat(payAmount).toFixed(8) },
      });
  ...
  const statusColor = (s: string, t: string): [string, string] => {
  ...
  const filterLabel = (f: FilterKey) => {
    const count = f === "all" ? credits.length : credits.filter(c => matchFilter(c, f)).length;
    return count > 0 ? `${FILTERS.find(x => x.key === f)?.label} (${count})` : FILTERS.find(x => x.key === f)?.label ?? f;
  };
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <View style={{ flexDirection: "row", gap: 10, padding: 16, paddingBottom: 0 }}>
        <View style={[styles.summaryCard, { backgroundColor: colors.saleBg, flex: 1 }]}>
          <Text style={{ fontFamily: "Inter_500Medium", fontSize: 10, color: colors.success }}>RECEIVABLE</Text>
          <Text style={{ fontFamily: "Inter_700Bold", fontSize: 17, color: colors.success, marginTop: 2 }}>${totalReceivable.toFixed(2)}</Text>
          <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, color: colors.success + "99", marginTop: 1 }}>to receive</Text>
        </View>
        <View style={[styles.summaryCard, { backgroundColor: colors.creditBg, flex: 1 }]}>
          <Text style={{ fontFamily: "Inter_500Medium", fontSize: 10, color: colors.credit }}>PAYABLE</Text>
          <Text style={{ fontFamily: "Inter_700Bold", fontSize: 17, color: colors.credit, marginTop: 2 }}>${totalPayable.toFixed(2)}</Text>
          <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, color: colors.credit + "99", marginTop: 1 }}>to pay out</Text>
        </View>
        <View style={[styles.summaryCard, { backgroundColor: colors.secondary, flex: 1 }]}>
          <Text style={{ fontFamily: "Inter_500Medium", fontSize: 10, color: colors.primary }}>RECEIVED</Text>
          <Text style={{ fontFamily: "Inter_700Bold", fontSize: 17, color: colors.primary, marginTop: 2 }}>${totalReceived.toFixed(2)}</Text>
          <Text style={{ fontFamily: "Inter_400Regular", fontSize: 10, color: colors.mutedForeground, marginTop: 1 }}>collected</Text>
        </View>
      </View>
        ...
        {FILTERS.map(f => (
          <TouchableOpacity
            ...
            style={{
              ...
              borderWidth: 1.5, borderColor: filter === f.key ? colors.primary : colors.border,
              flex: 1, alignItems: "center",
            }}
            onPress={() => setFilter(f.key)}
          >
            <Text style={{ fontFamily: "Inter_500Medium", fontSize: 11, color: filter === f.key ? "#FFF" : colors.mutedForeground }}>
              {filterLabel(f.key)}
            </Text>
          </TouchableOpacity>
        ))}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={{ alignItems: "center", padding: 40 }}>
              ...
              <Text style={{ fontFamily: "Inter_400Regular", color: colors.mutedForeground, marginTop: 12 }}>No credits</Text>
            </View>
          }
          renderItem={({ item: c }) => {
            const [tc, tb] = statusColor(c.status, c.type);
            const pct = parseFloat(c.amount) > 0 ? (parseFloat(c.paidAmount) / parseFloat(c.amount)) * 100 : 0;
            const statusLabel = c.status === "pending" ? "New" : c.status === "partial" ? "Pending" : "Received";
            return (
              <TouchableOpacity
                style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}
                onPress={() => { if (c.status !== "paid") { setSelected(c); setPayAmount(""); } }}
              >
                <View style={{ flexDirection: "row", alignItems: "flex-start", gap: 12 }}>
                  <View style={[styles.iconBox, { backgroundColor: tb }]}>
                    <Feather name={c.type === "receivable" ? "arrow-down-left" : "arrow-up-right"} size={18} color={tc} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontFamily: "Inter_700Bold", fontSize: 15, color: colors.text }}>{c.partyName}</Text>
                    <Text style={{ fontFamily: "Inter_400Regular", fontSize: 12, color: colors.mutedForeground }}>
                      {c.type === "receivable" ? "To Receive" : "To Pay"} • {c.partyType}
                    </Text>
                    ...
                    <View style={{ marginTop: 8 }}>
                      <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 4 }}>
                        ...
                        <Text style={{ fontFamily: "Inter_400Regular", fontSize: 11, color: colors.mutedForeground }}>Left: ${parseFloat(c.remainingAmount).toFixed(2)}</Text>
                      </View>
                      <View style={{ height: 4, backgroundColor: colors.border, borderRadius: 2 }}>
                        <View style={{ height: 4, width: `${Math.min(100, pct)}%` as unknown as number, backgroundColor: tc, borderRadius: 2 }} />
                      </View>
                    </View>
                  </View>
                  <View style={{ alignItems: "flex-end" }}>
                    ...
                    <View style={[{ paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6, marginTop: 4 }, { backgroundColor: tb }]}>
                      <Text style={{ fontFamily: "Inter_600SemiBold", fontSize: 11, color: tc }}>{statusLabel}</Text>
                    </View>
                  </View>
                </View>
              </TouchableOpacity>
          ...
          <View style={{ backgroundColor: colors.background, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24 }}>
            ...
            {selected && (
              <>
                ...
                <TextInput
                  style={{ borderWidth: 1.5, borderColor: colors.border, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, fontFamily: "Inter_400Regular", fontSize: 14, color: colors.text, backgroundColor: colors.input, marginBottom: 16 }}
                  value={payAmount}
                ...
                />
                <TouchableOpacity
                  style={{ backgroundColor: colors.credit, paddingVertical: 16, borderRadius: 14, alignItems: "center", marginBottom: 8 }}
                  onPress={handlePay}
                >
...
const styles = StyleSheet.create({
  summaryCard: { borderRadius: 12, padding: 12 },
